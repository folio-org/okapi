...
Okapi makes a HTTP POST to `/_/tenant/enable` if `_tenant` interface
version 2.0 is provided or `/_tenant` if `_tenant` interface version
1.0 is provided .  This is where the module may initialize its
database if necessary (for that one tenant), etc. With the POST
request a JSON object is passed: member `module_to` being the module
ID that is enabled.
...
request with path `/_/tenant/upgrade` if version 2.0 of interface
`_tenant` is provided ; or path `/_/tenant` if version 1.0 of
interface `_tenant` is provided. With the POST request a JSON object
is passed: member `module_from` being the module ID that we are
upgrading from and member `module_to` being the module ID that we are
upgrading 'to'. Note that the Module Descriptor of the target module
...
with path `/_/tenant/disable` if version 2.0 of interface `_tenant` is
